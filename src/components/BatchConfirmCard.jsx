...
export default function BatchConfirmCard({
  ...
  // 멀티데이: 1일치 템플릿만 표시
  const templateDay = batchDays[0]
  const templateEvents = templateDay?.events || []
    ...
    <div className="mt-2 border-2 border-green-300 dark:border-green-600 rounded-xl p-3 bg-white dark:bg-gray-800">
      ...
      {isMultiDay && (
        <div className="flex items-center gap-2 text-[11px] text-gray-500 dark:text-gray-400 mb-2 px-1">
          <Copy size={11} />
          <span>{templateDay.date} ~ {batchDays[batchDays.length - 1].date}</span>
          <span className="text-gray-400 dark:text-gray-500">
            ({templateEvents.length}{t('helperEventCount')} × {batchDays.length}{t('helperDayUnit')})
          </span>
        </div>
      )}
      ...
      <div className="max-h-72 overflow-y-auto space-y-1 mb-3">
        ...
        {templateEvents.map((event, eventIdx) => (
          <div
            key={eventIdx}
          ...
          >
            ...
            <button
              onClick={() => onRemoveItem(0, eventIdx)}
